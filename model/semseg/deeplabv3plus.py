...
class DeepLabV3Plus(nn.Module):
    ...
    def forward(self, x, fp_ratio=0):
        ...
        if fp_ratio!=0:
            dp1 = nn.Dropout(p=fp_ratio[0])
            dp2 = nn.Dropout(p=fp_ratio[1])
            c1 = torch.cat((dp1(c1[:b//2]), dp2(c1[b//2:])))
            c4 = torch.cat((dp1(c4[:b//2]), dp2(c4[b//2:])))

            # outs = self._decode(torch.cat((dp1(c1[:b//2]), dp2(c1[b//2:]))),
            #                     torch.cat((dp1(c4[:b//2]), dp2(c4[b//2:]))))
